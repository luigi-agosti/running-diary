...
public class ZoneIndipendentTimePicker extends Composite implements HasValueChangeHandlers<Date> {
    ...
    private class TimeSpinner extends ValueSpinner {
        ...
        protected String formatValue(long value) {
            ...
            if (dateTimeFormat != null) {
                return dateTimeFormat.format(new Date(dateInMillis), TIME_ZONE);
            }
            return "";
        }
    ...
    private SpinnerListener listener = new SpinnerListener() {
        public void onSpinning(long value) {
            ValueChangeEvent.fireIfNotEqual(ZoneIndipendentTimePicker.this, new Date(dateInMillis),
                    new Date(value));
        };
    };

    /**
     * @param use24Hours if set to true the {@link TimePicker} will use 24h
     *            format
     */
    public ZoneIndipendentTimePicker(boolean use24Hours) {
     ...
     *            format
     */
    public ZoneIndipendentTimePicker(Date date, boolean use24Hours) {
        this(date, use24Hours ? null : DateTimeFormat.getFormat("aa"), use24Hours ? DateTimeFormat
                .getFormat("HH") : DateTimeFormat.getFormat("hh"), DateTimeFormat.getFormat("mm"),
     ...
     *            hours field
     * @param minutesFormat the format to display the minutes. Can be null to
     *            hide minutes field
     * @param secondsFormat the format to display the seconds. Can be null to
     *            seconds field
     */
    public ZoneIndipendentTimePicker(Date date, DateTimeFormat amPmFormat, DateTimeFormat hoursFormat,
    ...
    public ZoneIndipendentTimePicker(Date date, DateTimeFormat amPmFormat, DateTimeFormat hoursFormat,
        ...
        if (amPmFormat != null) {
            TimeSpinner amPmSpinner = new TimeSpinner(date, amPmFormat, HALF_DAY_IN_MS, styles,
                    images);
            timeSpinners.add(amPmSpinner);
            horizontalPanel.add(amPmSpinner);
        }
        if (hoursFormat != null) {
            TimeSpinner hoursSpinner = new TimeSpinner(date, hoursFormat, HOUR_IN_MILLIS, styles,
                    images);
            timeSpinners.add(hoursSpinner);
            horizontalPanel.add(hoursSpinner);
        }
        if (minutesFormat != null) {
            TimeSpinner minutesSpinner = new TimeSpinner(date, minutesFormat, MINUTE_IN_MILLIS,
                    styles, images);
            timeSpinners.add(minutesSpinner);
            horizontalPanel.add(minutesSpinner);
        }
        if (secondsFormat != null) {
            TimeSpinner secondsSpinner = new TimeSpinner(date, secondsFormat, SECOND_IN_MILLIS,
                    styles, images);
